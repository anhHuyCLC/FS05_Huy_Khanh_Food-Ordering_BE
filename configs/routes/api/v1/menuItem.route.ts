...
export class MenuItemRouteV1 extends RailsRoute {
  public draw() {
    ...
    this.post("/restaurants/:restaurantId/menu-items", [action(AuthMiddleware), action(MenuItemControllerV1, "create")]);

    // PUT /menu-items/:menuItemId
    this.put("/menu-items/:menuItemId", [action(AuthMiddleware), action(MenuItemControllerV1, "update")]);

    // DELETE /menu-items/:menuItemId
    this.delete("/menu-items/:menuItemId", [action(AuthMiddleware), action(MenuItemControllerV1, "destroy")]);

    // PATCH /menu-items/:menuItemId/availability
    this.patch("/menu-items/:menuItemId/availability", [action(AuthMiddleware), action(MenuItemControllerV1, "updateAvailability")]);
  }
}
